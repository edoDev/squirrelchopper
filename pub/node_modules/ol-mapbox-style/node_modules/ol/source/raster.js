...
import _ol_ext_pixelworks_ from 'pixelworks';
...
_ol_source_Raster_.prototype.setOperation = function(operation, opt_lib) {
  this.worker_ = new _ol_ext_pixelworks_.Processor({
    operation: operation,
...
_ol_source_Raster_.prototype.updateFrameState_ = function(extent, resolution, projection) {
  ...
  frameState.focus = _ol_extent_.getCenter(extent);
 ...
 * @private
 */
_ol_source_Raster_.prototype.isDirty_ = function(extent, resolution) {
  var state = this.renderedState_;
  return !state ||
      this.getRevision() !== state.revision ||
      resolution !== state.resolution ||
      !_ol_extent_.equals(extent, state.extent);
};
...
_ol_source_Raster_.prototype.getImage = function(extent, resolution, pixelRatio, projection) {
  ...
  var width = Math.round(_ol_extent_.getWidth(currentExtent) / resolution);
  var height = Math.round(_ol_extent_.getHeight(currentExtent) / resolution);
  ...
  var imageCanvas = new _ol_ImageCanvas_(
 ...
 * @private
 */
_ol_source_Raster_.prototype.composeFrame_ = function(frameState, callback) {
 ...
 * @private
 */
_ol_source_Raster_.prototype.onWorkerComplete_ = function(frameState, callback, err, output, data) {
  if (err) {
    callback(err);
    return;
  }
